...
Notons ausi que la fonction `devtools::check` possède un argument `cran` prenant la valeur par défaut `TRUE`, ce qui signifie que la commande `R CMD check` est lancée avec l'option `as-cran`. Pour omettre cete option, il faut appeler la fonction devtools::check comme suit :
...
Tous ces formats d'archive peuvent être créés avec la [fonction `build`](https://devtools.r-lib.org/reference/build.html) du package `devtools`. Pour créer le package source (`.tar.gz`), aucun argument n'a a être fourni en entrée à la fonction. Celle-ci lance en fait la commande `R CMD build` et un fichier nommé `"nomPackage_numeroVersion.tar.gz"` (dans notre exemple  
 `manhattan_1.0.0.tar.gz`) est créé dans le répertoire **au-dessus** du répertoire principal du package.
...
-Hadley Wickham (2015). *R packages*. O'Reilly Media, Inc. 
...
- Hadley Wickham, Peter Danenberg, Gábor Csárdi et Manuel Eugster (2020). roxygen2: In-Line Documentation for R. R package version 7.1.0. URL https://CRAN.R-project.org/package=roxygen2
    + URL documentation en ligne https://roxygen2.r-lib.org/
- Hadley Wickham et Jennifer Bryan (2019). usethis: Automate Package and Project Setup. R package version 1.5.1. https://CRAN.R-project.org/package=usethis
    + URL documentation en ligne https://usethis.r-lib.org/
- Hadley Wickham, Jim Hester et Winston Chang (2020). devtools: Tools to Make Developing R Packages Easier.
...
- Jennifer Bryan, Jim Hester et Hadley Wickham (2019). Tutoriel présenté dans le cadre de la conférence UseR 2019 intitulé « Package Development ». URL https://github.com/jennybc/pkg-dev-tutorial
- Matt Dray (2019). Tutoriel web intitulé « Build an R package with {usethis} ». URL https://www.rostrum.blog/2019/11/01/usethis/
- Sharla Gelfand (2019). Tutoriel web intitulé « usethis for reporting ». URL https://sharla.party/post/usethis-for-reporting/
