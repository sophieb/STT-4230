...
- bouton **Knit** (accompagné d'un logo représentant une pelote de laine avec des aiguilles à tricoter).
...
Notons que d'autres fonctions permettent de formater une matrice ou un data frame R pour créer un tableau dans un document rédigé en R Markdown. Quelques-unes sont mentionnées dans le document https://github.com/rstudio/cheatsheets/blob/master/rmarkdown-2.0.pdf. La mise en forme du tableau obtenu diffère légèrement d'une fonction à l'autre.

Si le document final doit être en HTML uniquement, le package `formattable` peut être utilisé pour formater un data frame à afficher sous forme de tableau. Il offre un large éventail d'options de formatage.
