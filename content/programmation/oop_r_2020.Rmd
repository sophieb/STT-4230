...
any(methods(class = "numeric") == "numeric")
```
...
De plus, spécifier l'argument `add = TRUE` dans l'appel à la fonction `on.exit` [est recommandé](https://adv-r.hadley.nz/functions.html#on-exit) afin de permettre le cumul des instructions de fin d'exécution. Notons qu'avec un seul appel à `on.exit` dans le corps d'une fonction, c'est argument n'a cependant aucun impact. 
