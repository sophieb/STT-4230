...
Il existe en R d'autres fonctions de haut niveau, qui appliquent itérativement une fonction sur les éléments d'un objet. Pour les intéressés, la fiche d'aide ouverte par la commande [`help(funprog)`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/funprog.html) présente ces fonctions (`Map`, `Filter`, `Reduce`, etc.). Un package R se spécialise aussi dans ce genre de fonctions, il s'agit du [package `purrr`](https://purrr.tidyverse.org/).
