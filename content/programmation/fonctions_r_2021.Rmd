...
Parfois, même si un argument ne possède pas de valeur par défaut dans la liste des arguments, une valeur lui est attribuée dans le corps de la fonction si l'utilisateur appelle la fonction sans assigner de valeur à l'argument. Prenons l'exemple de la [fonction `sample`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/sample.html) vue dans les [notes sur les calculs statistiques en R](https://stt4230.rbind.io/calculs/calculs_stat_r/#fonction-sample) pour illustrer cette possibilité.
...
La fonction `sample` fait appel à la [fonction `missing`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/missing.html) pour tester si une valeur a été fournie en entrée à `size`. Si ce n'est pas le cas, une valeur est assignée à `size`.

Cette façon de faire n'est pas idéale. Il est plus clair pour l'utilisateur de voir les valeurs par défaut dans la liste des arguments. Le comportement par défaut de la fonction `sample` si l'argument `size` n'est pas fourni est tout de même expliqué dans la section intitulée «\ *Details*\ » de la [fiche d'aide de la fonction `sample`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/sample.html)).
...
Une valeur par défaut peut utiliser les valeurs prises par d'autres arguments. Prenons par exemple la [fonction `sample.int`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/sample.html).
...
Pour éviter une valeur par défaut dont l'expression est trop complexe, certains auteurs de fonctions R utilisent `NULL` comme valeur par défaut dans la liste des arguments, puis définissent la vraie valeur par défaut dans le corps de la fonction. Par exemple, la [fonction `cut`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/cut.html) vue dans les [notes sur le prétraitement de données en R](https://stt4230.rbind.io/manipulation_donnees/pretraitement_donnees_r/#fonction-cut) implémente cette pratique pour son argument `labels`. 
...
### Arguments acceptant seulement quelques chaînes de caractères spécifiques
...
Les deux utilités de l'[argument `...`](https://stt4230.rbind.io/introduction/base_r/#largument-...-et-ses-deux-utilit%C3%A9s) ont été mentionnées lors du premier cours. Nous pouvons recourir à cet argument dans nos propres fonctions, en exploitant l'une ou l'autre de ses utilités.
...
En résumé, lorsque l'argument `...` sert à recevoir en entrée un nombre indéterminé de valeurs à traiter, les [instructions suivantes peuvent être utiles pour manipuler, dans le corps de la fonction, les valeurs attrapées par `...`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/dots.html)\ :
...
## Retour de plusieurs éléments dans une liste
...
Comme pour les fonctions `Hello World`, `test_appel` et `test_trois_points`, l'évaluation de l'appel à une fonction peut produire des «&nbsp;[effets de bord](https://fr.wikipedia.org/wiki/Effet_de_bord_(informatique))&nbsp;» (en anglais *side effects*). Ces effets de bords peuvent être en réalité le but principal de la fonction.
...
```{r echo = FALSE, out.width = "25%", fig.align = 'center', fig.cap = "Exemple de chemin de recherche lors de l'évaluation de l'appel à une fonction R"}
knitr::include_graphics("fig/chemin_fonction.png")
```


## Bonnes pratiques dans la création d'une fonction
...
Nous allons créer ensemble une fonction qui compte combien de nombres entiers impairs contient un vecteur numérique (cet exemple est tiré de [@matloff]).
...
4. **Documenter** la fonction\ : les informations minimales à fournir sont
