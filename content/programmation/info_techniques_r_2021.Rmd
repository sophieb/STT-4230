...
La dernière ligne de la sortie R générée identifie l'environnement de provenance de la fonction. Il s'agit de l'espace de nom du package `generics`. Il faut savoir ici que le package `dplyr` importe la fonction `union` du package nommé `generics`. Ainsi, la fonction `union` atteinte est bel et bien celle provenant du package `dplyr`. (Nous reviendrons sur les espaces de noms dans les [notes sur l'utilisation de packages R](https://stt4230.rbind.io/packages/utilisation_packages_r/#espace-de-noms-namespace-dun-package).)
