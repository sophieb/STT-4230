...
Voici d'emblée une vue d'ensemble de plusieurs des fonctions présentées ici.

```{r echo = FALSE, out.width = "90%", fig.pos = 'h', fig.align = 'center', fig.cap = "Vue d'ensemble de plusieurs fonctions et opérateurs mathématiques et statistiques en R"}
...
La commande précédente effectue 4 additions, une pour chacun des 4 éléments du plus long des deux vecteurs dans l'opération, soit ici le deuxième. Le premier vecteur est plutôt de longueur 2. R répète donc ses éléments pour créer un vecteur aussi long que le deuxième
...
#### Règle de recyclage lorsque la longueur de l'objet le plus court n'est pas multiple de la longueur de l'objet le plus long

Lorsque la longueur de l'objet le plus court n'est pas multiple de la longueur de l'objet le plus long, la règle de recyclage fonctionne quand même. R recycle l'objet le plus court assez de fois pour arriver à un objet de longueur égale ou supérieure à l'objet le plus long. Ensuite, si l'objet recyclé est plus long que l'autre objet, il est tronqué de façon à ce que les deux objets aient la même longueur.
...
Pour connaître l'ordre utilisé dans une session R, la commande suivante est utile :
...
Ainsi, dans ma session R, les caractères spéciaux sont inférieurs aux chiffres et aux lettres, les chiffres sont inférieurs aux lettres et les lettres minuscules sont inférieures aux lettres majuscules.

Aussi en cas d'inégalité dans le nombre de caractères, les chaînes contenant moins de caractères sont considérées inférieures aux chaînes en contenant plus, comme l'illustre ces exemples.
...
Ainsi, l'opération « a ou b » retourne vrai uniquement si a et b sont vrais.
...
pmax(x, 1:7)
```
Elle est utile pour tronquer des valeurs. Par exemple, pour tronquer à zéro des valeurs, on peut procéder comme suit.
```{r}
pmax(x, 0)
```
...
- la manipulation simultanée des éléments d'une liste, 
...
La fonction `apply` applique une fonction sur toutes les lignes ou toutes les colonnes d'une matrice. 
...
Pour le calcul de sommes et de moyennes, il existe des fonctions raccourcies plus rapides qu'un appel à la fonction `apply` : `rowSums`, `colSums`, `rowMeans`, `colMeans`. Par exemple

```{r}
rowMeans(mat, na.rm = TRUE)
```
est équivalent à `apply(mat, 1, mean, na.rm = TRUE)`,
```{r}
...
Les fonctions `lapply`, `sapply` et `mapply` prennent en entrée une liste ou un data.frame. Elles appliquent une fonction sur chaque élément dans la liste, ou chaque colonne du data frame.
...
Il aurait aussi été possible de solutionner le problème de l'extraction des derniers mots des phrases dans `phrases` en utilisant la fonction `mapply`. La différence entre cette fonction et les fonctions `sapply` et `lapply` est qu'elle peut fournir à fonction la `FUN` des arguments supplémentaires prenant des valeurs qui diffèrent selon les sous-objets.
...
La boucle cachée derrière à cet appel à la fonction `mapply` est la suivante : pour `i` allant de 1 à `r length(phrases)`, soit le nombre total d'éléments dans la liste `phrases`, l'extraction suivante est effectuée.
...
## Synthèse des informations sur les fonctions de la famille des `apply`

Voici un tableau synthèse des fonctions présentées dans cette section.

: Synthèse des spécifications des fonctions de la famille des `apply`

Fonction | Objet en entrée     | Formation des sous-objets  | Format de la sortie 
--------------------------|---------------------------|-----------------------------------------|-----------------------------
`apply`  | array (matrice) | selon une ou des dimensions        | vecteur, array ou liste
--------------------------|---------------------------|-----------------------------------------|-----------------------------
`lapply` | liste ou data frame              | chaque élément de la liste | liste
`sapply` et `mapply` | liste ou data frame               | chaque élément de la liste | simplifié par défaut
--------------------------|---------------------------|-----------------------------------------|-----------------------------
...
`mapply` permet de passer des arguments de `FUN` différents selon les sous-objets, alors que ces arguments ne peuvent pas varier avec `sapply`.


Voici la description des arguments à donner en entrée à ces fonctions :

- 1^e^ argument (sauf pour `mapply`) : objet à séparer et sur lequel appliquer la fonction;

- argument suivant : information pour spécifier comment séparer l'objet en sous-objets (sauf pour les fonctions prenant en entrée une liste, car dans ce cas les sous-objets sont les éléments de la liste);

- argument suivant (celui nommé `FUN`) : la fonction à appliquer (les sous-objets lui seront fournis comme premier argument);

- `...` : il est possible de passer des arguments supplémentaires à la fonction à appliquer simplement en les donnant en argument à la fonction de la famille des `apply` grâce aux `...` (rappel : il s'agit de la deuxième utilité de l'argument `...` mentionnée dans les notes sur les [concepts de base en R](https://stt4230.rbind.io/introduction/base_r/#largument-...-et-ses-deux-utilites.)).
...
L'opérateur `%in%` est cependant plus rapide pour créer ce vecteur logique. Il compare les éléments d'un vecteur (placé avant l'opérateur) aux éléments d'un ensemble présenté sous la forme d'un vecteur (placé après). Il retourne `TRUE` pour un élément égal à n'importe lequel des éléments de l'ensemble, `FALSE` sinon. 
...
Pour résumer, les opérateurs et fonctions R utiles à l'écriture de conditions logiques sont les suivants :
...
- opérateurs logiques : `!` (négation), `|` (ou) et `&` (et).
...
Finalement, les fonctions `is.numeric`, `is.character`, `is.logical`, `is.vector`, `is.matrix`, `is.data.frame`, `is.factor`, `is.null`, `is.function`, etc., testent une condition et retournent toujours un logique de longueur 1.

Pour résumer, les opérateurs et fonctions suivantes retournent toujours un logique de longueur 1 :

- opérateurs logiques (qui utilisent seulement le premier élément des vecteurs) : `||` (ou), `&&` (et);
- fonctions qui condensent un vecteur logique en une seule valeur logique : `all, any`;
- fonctions de vérification de type :  
`is.`(`numeric`/`character`/`logical`/`vector`/`matrix`/`array`/`list`/`data.frame`/`factor`/`null`/...) (il en existe beaucoup!).
...
Le package [`dplyr`](https://CRAN.R-project.org/package=dplyr) offre des fonctions permettant de réaliser quelques-unes des manipulations de données vues dans ce cours ainsi que le cours précédent, par exemple :
