...
*Note préliminaire\ : Lors de leur dernière mise à jour, ces notes ont été révisées en utilisant R version 4.0.3, le package `dplyr` version 1.0.3, le package `tidyr` version 1.1.2, le package `data.table` version 1.13.6, le package `stringr` version 1.4.0 et le package `lubridate` version 1.7.9.2. Pour d'autres versions, les informations peuvent différer.*
...
## Modification ou ajout de variables
...
Nous savons que l'instruction `cars$dist`, servant à identifier la colonne à modifier, aurait pu être écrite de bien d'autres façons, notamment\ : 
...
Je considère cependant que l'opérateur `$` procure la syntaxe la plus simple pour un remplacement de variable dans un data frame. Le jeu de données `cars` a été modifié comme suit.
...
La [fonction `transform`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/transform.html) offre une autre façon d'écrire ce genre d'assignation. La commande suivante produit le même résultat que la dernière assignation.

```{r}
cars <- transform(cars, dist = dist * 0.3048)
```

Il faut donner comme premier argument à la fonction `transform` le data frame dans lequel nous souhaitons effectuer une transformation de variable. Les arguments suivants sont des assignations de nouvelles valeurs. Dans ces assignations, il est possible de référer aux colonnes du data frame fourni comme premier argument par leur nom sans les précéder de `nom_du_data_frame$` et sans les encadrer de guillemets, comme avec la [fonction `subset`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/subset.html), vue dans le [cours sur les structures de données en R](https://stt4230.rbind.io/manipulation_donnees/structures_r/#fonctions-dextraction).


#### Remarque concernant la présence de deux objets nommés `cars` dans le chemin de recherche  {-}

\mbox{}

Nous avons maintenant deux objets nommés `cars` dans notre chemin de recherche, parce que nous venons de modifier un data frame provenant d'un package chargé. Le data frame modifié, est situé dans notre environnement de travail. Alors que le data frame `cars` original est toujours dans l'environnement du package `datasets`. Celui-là n'a pas été modifié.
...
Lorsque nous demandons à R d'accéder à l'objet nommé `cars`, il cherche ce nom dans les environnements de son chemin de recherche, un environnement à la fois, en respectant l'ordre des environnements dans la liste précédente. L'environnement `".GlobalEnv"`, soit notre environnement de travail, est le premier de la liste. Lorsque R trouve le nom, il cesse sa recherche. Ainsi, toute commande dans la console contenant le nom `cars`, accédera maintenant à l'objet nommé `cars` de notre environnement de travail.

```{r}
str(cars)
```

Il est possible de forcer R à aller chercher l'objet `cars` dans le package `datasets` avec l'opérateur `::`.
...
#### Ajout plutôt que modification de variables  {-}

Au lieu d'écraser une colonne d'un data frame en la remplaçant par un autre objet, comme nous venons de le faire, il sera souvent préférable d'ajouter les nouvelles variables créées tout en conservant les anciennes. Dans ce cas, il suffit de concaténer les nouvelles variables au data frame avec la [fonction `cbind`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/cbind.html) ou [`data.frame`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/data.frame.html) (vues plus loin), ou encore d'assigner les nouvelles variables à des colonnes portant des noms qui ne sont pas initialement présents dans le data frame. Par exemple, pour ajouter une variable contenant la distance exprimée en mètres au data frame `cars`, nous pourrions procéder comme suit.

```{r}
cars <- datasets::cars  # pour repartir avec la version originale du jeu de données
cars$dist_m <- cars$dist * 0.3048
head(cars)
```

Étant donné que le jeu de données `cars` ne contenait pas à l'origine de colonne nommée `dist_m`, une nouvelle colonne portant ce nom a été créée et ajoutée après les colonnes déjà présentes dans le data frame.

La fonction `transform` permet aussi d'ajouter de nouvelles variables par une assignation à un nouveau nom de colonne.

**Remarque**\ : Il y a souvent plusieurs façons de réaliser une même tâche en R. Personnellement, je tends à adopter la façon qui présente le meilleur compromis entre un code le plus facile à comprendre possible et un code le plus court possible. Ici, l'assignation à un nouveau nom de colonne avec l'opérateur `$` me semble la meilleure option, mais à vous de choisir la méthode que vous préférez.
...
La [fonction `ifelse`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/ifelse.html) est souvent utile pour transformer des variables. Cette fonction prend comme premier argument un vecteur de valeurs logiques. Cet argument est nommé `test`. Elle retourne en sortie un vecteur de même longueur que `test`. Ce vecteur est formé des valeurs dans le deuxième argument (`yes`) aux positions pour lesquelles `test` comporte une valeur `TRUE`, et des valeurs dans le troisième argument (`no`) aux autres positions. Voici un petit exemple pour comprendre le fonctionnement de cette fonction.
...
Par exemple, supposons que nous souhaitons créer une nouvelle variable dans le jeu de données `cars` contenant la vitesse en km/heure plutôt qu'en miles/heure, mais dans laquelle les valeurs de distance en miles à l'heure inférieures à 10 sont ramenées à des valeurs manquantes. Nous pourrions créer cette variable comme suit (1.60934 est le facteur de conversion entre un mile à l'heure et un kilomètre à l'heure). 

```{r}
cars$speed_km <- ifelse(test = cars$speed < 10, yes = NA, no = cars$speed * 1.60934)
head(cars, n = 10)
```
...
```{r}
cars[c(5:7,38:40), c("speed", "speed_cat")]
```
...
La principale fonction pour catégoriser une variable numérique en R est la [fonction `cut`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/cut.html). En voici un exemple d'utilisation.
```{r cut}
cars$speed_cat_2 <- cut(cars$speed, breaks = c(-Inf, 10, 20, Inf), right = FALSE)
str(cars)
```
L'argument `right` sert à préciser si les limites de droite des intervalles de catégorisation sont incluses dans ceux-ci ou non. Ici, la valeur `FALSE` fournie à cet argument a pour conséquence que les intervalles sont ouverts à droite\ : $[-\infty, 10), [10, 20), [20, \infty)$.

La nouvelle variable obtenue est un facteur, avec des libellés de niveaux différents de ceux de la variable `speed_cat`. 
```{r}
cars[c(5:7,38:40), c("speed", "speed_cat", "speed_cat_2")]
```
Nous pouvons contrôler ces libellés avec l'argument `labels` de la fonction `cut`, comme dans cet exemple.
```{r}
cars$speed_cat_3 <- cut(
  cars$speed, 
  breaks = c(-Inf, 10, 20, Inf), 
  labels = c("faible", "moyenne", "grande"),
  right = FALSE
)
cars[c(5:7,38:40), c("speed", "speed_cat", "speed_cat_2", "speed_cat_3")]
```


### Fonction `case_when` du package `dplyr`

L'équivalent de la fonction `cut` dans l'univers du `tidyverse` est la [fonction `case_when` du package `dplyr`](https://dplyr.tidyverse.org/reference/case_when.html). À titre d'exemple, le dernier appel à `cut` est reproduit comme suit avec `case_when`.

```{r}
cars$speed_cat_4 <- case_when(
...
Contrairement à la fonction `cut`, `case_when` retourne un vecteur plutôt qu'un facteur. 



### Fonction `ave` 

Si nous souhaitions remplacer les identifiants des nouvelles catégories par une statistique calculée sur les valeurs observées dans chaque catégorie, nous utiliserions la [fonction `ave`](https://stat.ethz.ch/R-manual/R-patched/library/stats/html/ave.html), comme dans cet exemple.

```{r ave}
cars$speed_ave <- ave(cars$speed, cars$speed_cat, FUN = mean)
```

```{r}
cars[c(5:7,38:40),c("speed", "speed_cat", "speed_cat_2", "speed_ave")]
```

Pour une observation, la valeur de la nouvelle variable `speed_ave` est la moyenne de toutes les vitesses observées dans la même catégorie que celle de l'observation en question.
...
Ici, le résultat produit possède seulement 3 lignes, pour les trois catégories distinctes. La sortie de l'appel à la fonction `ave` précédent était plutôt de longueur 50, soit la longueur de `cars$speed`.
...
La [fonction `scale`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/scale.html) permet de normaliser par colonne les valeurs numériques dans une matrice ou un data frame.

Par exemple, revenons au jeu de données `cars` d'origine.
...
Les valeurs pour les deux variables dans le jeu de données `cars` ne sont pas sur la même échelle. En effet, leurs moyennes et écarts-types empiriques ne sont pas similaires. Utilisons la fonction `apply`, vue dans les notes sur les [calculs par sections d'un objet R](https://stt4230.rbind.io/calculs/calculs_apply_r/#fonction-apply), pour calculer ces statistiques par variable (= par colonne). 
...
**Note**\ : La [fonction `sweep`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/sweep.html) permet aussi de faire simultanément une transformation pour chacune des colonnes d'une matrice. Elle généralise en fait `scale` en permettant d'effectuer la transformation sur n'importe laquelle des dimensions d'un array. Cette fonction ne sera cependant pas approfondie ici.   
...
Il s'agit de données concernant les recettes (`Gross`) des films ayant généré le plus de revenus dans les cinémas américains.

### Fonction `paste`

La [fonction `paste`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/paste.html) sert à concaténer des éléments de vecteurs. Elle retourne toujours un résultat sous le format caractère. Elle n'est pas illustrée ici, car elle a été présentée dans les [notes sur les structures de données en R](https://stt4230.rbind.io/manipulation_donnees/structures_r/#fonctions-paste-et-paste0).

Rappelons que la fonction `paste0` est en quelque sorte une version de la fonction `paste` avec l'argument `sep = ""` (aucun caractère comme séparation entre les chaînes de caractères à concaténer).


### Fonction `nchar`

La [fonction `nchar`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/nchar.html) permet de compter le nombre de caractères dans des chaînes de caractères. Elle a aussi été illustrée dans les [notes sur les structures de données en R](https://stt4230.rbind.io/manipulation_donnees/structures_r/#vecteurs-de-cha%C3%AEnes-de-caract%C3%A8res).  

### Fonctions `toupper`, `tolower` et `casefold`

Les [fonctions `toupper` et `tolower`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/chartr.html) transforment tous les caractères alphabétiques en majuscules et en minuscules respectivement.
...
La [fonction `casefold`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/chartr.html), utilisée comme exemple de fonction dans les [notes sur les concepts de base en R](https://stt4230.rbind.io/introduction/base_r/#fonctions-r), permet de faire les deux transformations de casse, soit transformer tous les caractères alphabétiques en majuscules (avec l'argument `upper = TRUE`) ou transformer tous les caractères alphabétiques en minuscules (avec l'argument `upper = FALSE`).
...
Le résultat de la fonction `strsplit` est toujours une liste de la même longueur que le vecteur assigné à l'argument `x`. Chaque élément de la liste est un vecteur comprenant les sous-chaînes de caractères obtenues de la séparation. Ici, ces vecteurs ont le plus souvent une longueur de 2. Par exemple pour le premier film du jeu de données, `r boxoffice$Movie_Distributor[1]`, on obtient le vecteur `[`r paste0("\"", res_split[[1]], "\"")`]`. Par contre, certains titres de film comprennent un élément entre parenthèses de plus, par exemple `"`r boxoffice$Movie_Distributor[which(sapply(res_split, length) == 3)[1]]`"`. Pour eux, la longueur du vecteur est 3. 

La commande suivante permet d'extraire le dernier élément de chacun des vecteurs se trouvant dans `res_split`. Elle utilise la fonction `sapply` vue dans le cours sur les [calculs par sections d'un objet R](https://stt4230.rbind.io/calculs/calculs_apply_r/#fonctions-lapply-sapply-et-mapply).

```{r}
distributeur <- sapply(res_split, tail, n = 1)
...
Afin d'avoir vraiment isolé le nom du distributeur, il ne reste plus qu'à retirer la parenthèse fermante à la fin des chaînes de caractères dans `distributeur`. La [fonction `substr`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/substr.html) permet d'extraire une partie de chaînes de caractères en spécifiant les positions, dans les chaînes, des caractères à conserver.
...
Pour la tâche spécifique de retirer des accents, il est en fait encore plus simple d'utiliser la [fonction `iconv`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/iconv.html). Celle-ci permet de convertir l'encodage de chaîne de caractères. Un bon truc pour retirer des accents de chaînes de caractères, peu importe la langue utilisée dans ces chaînes, est de convertir vers l'encodage `"ASCII//TRANSLIT"`. Voici un exemple.
...
Nous allons convertir les données dans la colonne `Release_Date` vers la classe `"Date"` de R avec la [fonction `as.Date`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/as.Date.html). Cependant, cette conversion fonctionnera uniquement si les paramètres régionaux (en anglais *locale*) de notre session R sont en anglais, puisque les dates comportent des abréviations de noms de mois en anglais. Ainsi, commençons par vérifier quel paramètre régional pour le temps utilise notre session R avec la [fonction `Sys.getlocale`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/locales.html).
...
Les valeurs comprises comme argument `locale` dépendent du système d'exploitation. Sous un système d'exploitation macOS ou Linux, tentez une des valeurs suivantes\ : `"en_CA"`, `"en_CA.UTF-8"` ou `"en_CA.utf8"` (ou la version US de ces valeurs, soit `"en_US"`, `"en_US.UTF-8"` ou `"en_US.utf8"`).
...
Pour calculer le nombre d'années entre aujourd'hui et la date de sortie des films, tentons d'utiliser la [fonction `difftime`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/difftime.html). D'abord, obtenons la date courante avec la [fonction `Sys.Date`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/Sys.time.html). 
...
Une date peut se présenter sous plusieurs formats, explicités dans la [fiche d'aide nommée `strptime`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/strptime.html). 

```{r eval = FALSE}
help(strptime)
```

Grâce à ces différents formats, nous pourrions par exemple extraire le mois de sortie d'un film grâce à la [fonction `format`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/strptime.html) comme suit.
...
**Note**\ : Il existe d'autres classes dédiées à la manipulation de dates en R qui permettent d'inclure l'heure dans la date (heure, minute, seconde)\ : [`"POSIXlt"` et `"POSIXct"`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/DateTimeClasses.html). Avec ces classes, la prise en compte des fuseaux horaires est primordiale. Nous n'approfondirons pas l'utilisation de ces classes.  Seule une petite partie des possibilités de manipulation de dates en R est abordée ici.
...
library(lubridate)
boxoffice$Date_2 <- mdy(boxoffice$Release_Date, locale = "English")
boxoffice$Age_2 <- time_length(interval(start = boxoffice$Date_2, end = now()), "years")
head(subset(boxoffice, select = c(Movie_Distributor, Date, Date_2, Age, Age_2)))
```

```{r echo = FALSE, message = FALSE, R.options = list(width = 85)}
library(lubridate)
boxoffice$Date_2 <- mdy(boxoffice$Release_Date, locale = "English")
boxoffice$Age_2 <- time_length(interval(start = boxoffice$Date_2, end = aujourdui), "years")
head(subset(boxoffice, select = c(Movie_Distributor, Date, Date_2, Age, Age_2)))
```

La fonction `time_length` tient compte des années bissextiles. Elle peut aussi calculer des durées en mois tout en considérant les durées variables de ceux-ci. Remarquons que l'argument `locale` de la fonction `mdy` nous a permis de spécifier un paramètre régional s'appliquant uniquement à la conversion à effectuer. La modification d'un paramètre régional global de la session R n'est pas nécessaire avec les fonctions du packages `lubridate`. Des références sont fournies à la fin de ce document pour en apprendre davantage sur le package `lubridate`.
...
- le retrait d'observations dupliquées ou contenant des données manquantes,
...
Pour illustrer des manipulations, reprenons le data frame `data_ex` créé dans le cours concernant la [lecture et l'écriture dans des fichiers externes à partir de R](https://stt4230.rbind.io/manipulation_donnees/lecture_ecriture_r/#lecture-de-fichiers), en modifiant une valeur afin d'introduire une observation dupliquée.

```{r}
de_1 <- c(2, 3, 4, 1, 2, 3, 5, 6, 5, 4)
de_2 <- c(1, 4, 2, 3, 1, 4, 6, 2, 5, 3)  # 5e valeur pas comme dans les notes précédentes
...
Dans le jeu de données `data_ex`, les lignes 1 et 5 de sont identiques. La fonction `duplicated` identifie la 5e observation comme une duplication d'une autre observation.
```{r}
duplicated(data_ex)
```
...
### Retrait des observations avec données manquantes
...
- pour identifier les observations sans données manquantes\ : `complete.cases`,
- pour retirer les observations contenant au moins une donnée manquante\ : `na.omit`.

Dans le jeu de données `data_ex`, les lignes 3 et 7 contiennent chacune une donnée manquante. La fonction `complete.cases` identifie ces lignes comme n'étant pas des cas complets (`FALSE` retourné pour les lignes contenant des `NA`).
```{r}
complete.cases(data_ex)
```
Ces deux observations peuvent facilement être retirées avec la fonction `na.omit` comme suit.
```{r}
na.omit(data_ex)
```
La fonction `na.omit` a déjà été vue dans les [notes sur les calculs statistiques en R](https://stt4230.rbind.io/calculs/calculs_stat_r/#traitement-des-donn%C3%A9es-manquantes-et-argument-na.rm)). Cette fonction ajoute des attributs à l'objet retourné pour identifier les observations omises.
```{r}
data_ex_no_NA <- na.omit(data_ex)
str(data_ex_no_NA)
```
Les fonctions `complete.cases` et `na.omit` fonctionnent aussi avec des vecteurs, comme le montre ces exemples.
```{r}
complete.cases(c(1, 3, 2, NA, 2, 1))  # équivalent à !is.na(c(1, 3, 2, NA, 2, 1))
na.omit(c(1, 3, 2, NA, 2, 1))
```
La fonction `is.na` (vue dans les [notes sur les calculs mathématiques en R](https://stt4230.rbind.io/calculs/calculs_math_r/#fonctions-de-comparaison-pour-constantes-sp%C3%A9ciales)), utilisée avec l'opérateur `[`, permet aussi de retirer les données manquantes d'un vecteur.
```{r}
x <- c(1, 3, 2, NA, 2, 1)
x[!is.na(x)]
```
Remarquons que cette solution de rechange à `na.omit` n'ajoute pas d'attributs au résultat.
...
Pour illustrer l'utilisation de ces outils dans un contexte de sélection de sous-ensembles de données, voyons comment conserver du jeu de données `data_ex` seulement les résultats de lancers de dés effectués par Kim et ne comprenant aucune donnée manquante. Dans ce sous-ensemble, la variable `lanceur` prendra toujours la même valeur. Elle ne sera donc plus vraiment pertinente et sera retirée.
...
```{r}
data_ex[data_ex$lanceur == "Kim", ]
```

Remarquons que l'observation pour laquelle la valeur de la variable `lanceur` est manquante a été conservée. Ensuite, la façon la plus simple de retirer les lignes comprenant au moins une donnée manquante est d'utiliser la fonction `na.omit` mentionnée précédemment.
...
Par exemple, nous pouvons sélectionner les observations de `data_ex` pour lesquelles la valeur de la variable `lancer` est `"Kim"` avec la fonction `filter` comme suit.
...
Notons que la fonction `drop_na` permet de limiter la recherche de valeurs manquantes à un sous-ensemble de variables, ce qui n'est pas possible avec `na.omit`.
...
La pratique courante avec les packages du `tidyverse` est cependant d'utiliser l'[opérateur «\ pipe\ » `%>%`](https://magrittr.tidyverse.org/) du package `magrittr` pour rendre les commandes plus lisibles. En utilisant cet opérateur, la commande aurait l'allure suivante.

```{r}
data_ex %>% filter(lanceur == "Kim") %>% drop_na() %>% select(-lanceur) 
```

Je ne fais que mentionner cet opérateur ici. Il est recommandé, mais pas obligatoire, de l'employer lors de l'utilisation de packages du `tidyverse`. L'utilisation de cet opérateur est expliqué dans les [notes sur les bonnes pratiques de programmation en R](https://stt4230.rbind.io/amelioration_code/bonnes_pratiques_r/#op%C3%A9rateur-pipe).
...
Après avoir transformé le data frame `data_ex` en data table, 

```{r}
data_ex_dt <- as.data.table(data_ex)
```
...
La sélection de variables dans un data table s'effectue en fournissant une valeur à l'argument `j`. Par exemple, nous pouvons sélectionner les variables `de1` et `de2` de `data_ex_dt` comme suit (commande non évaluée),
...
Remarquons que l'omission de variables s'indique avec l'opérateur `!` avec `data.table`, plutôt qu'avec `-`.
...
Supposons que nous avons en main deux jeux de données contenant différentes observations pour les mêmes variables. Par exemple, en plus du data frame `data_ex`, disons que nous avons les observations supplémentaires suivantes.
...
Pour concaténer les jeux de données `data_ex` et `supp` en un seul jeu de données, nous pourrions utiliser la [fonction `rbind`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/cbind.html) comme suit\ :
...
La même tâche aurait pu être effectuée en utilisant en appelant la [fonction `cbind`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/cbind.html) ou [`data.frame`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/data.frame.html) comme suit.

```{r eval = FALSE}
cars <- cbind(cars, dist_m = cars$dist * 0.3048)
# ou encore
cars <- data.frame(cars, dist_m = cars$dist * 0.3048)
```
...
Pour concaténer en colonnes les jeux de données `data_ex` et `data_ex_lanceurs`, nous pourrions utiliser la fonction `cbind` ou `data.frame` comme suit.
...
Une fusion par association selon la valeur de la variable identifiant le lanceur entre les data frames `data_ex` et `lanceurs` peut s'effectuer comme suit.
...
```{r}
supp_2 <- data.frame(supp, lateralite = c("gaucher", "droitier"))
supp_2
...
Le package `data.table` contient la [fonction `rbindlist`](https://rdatatable.gitlab.io/data.table/reference/rbindlist.html), qui est une solution de rechange à `rbind` du R de base ou `bind_rows` de `dplyr`. Elle a les mêmes capacités que cette dernière.
...
Une des fonctions les plus simples pour effectuer un  ordonnancement d'éléments est la [fonction `rev`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/rev.html), qui renverse l'ordre des éléments d'un vecteur (ou un facteur).
...
Pour effectuer l'ordonnancement des éléments d'un vecteur (ou un facteur) selon les valeurs des données en éléments, de façon à replacer les éléments en ordre croissant ou décroissant de valeur, il faut plutôt utiliser la [fonction `sort`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/sort.html). Par exemple, ordonnons des valeurs numériques en ordre décroissant.
...
Nous avons vu dans les [notes sur les calculs mathématiques en R](https://stt4230.rbind.io/calculs/calculs_math_r/#op%C3%A9rateurs-de-comparaison) comment connaître l'ordre utilisé par notre session R pour classer des caractères. Rappelons qu'il suffit de fournir à la fonction `sort` un vecteur contenant tous les caractères potentiels, par exemple.
...
L'[opérateur d'indiçage `[`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/Extract.html) a pratiquement toujours été utilisé jusqu'à maintenant comme un outil d'extraction d'éléments. Dans les [notes sur les structures de données](https://stt4230.rbind.io/manipulation_donnees/structures_r/#op%C3%A9rateurs-dindi%C3%A7age), il a été mentionné que cet opérateur acceptait différents types de données en entrée pour identifier les éléments à extraire\ : entiers positifs, entiers négatifs, chaînes de caractères ou valeurs logiques. Deux de ces types de données permettent également de réordonner les éléments\ : les entiers positifs et les chaînes de caractères.

Par exemple, pour modifier l'ordre des colonnes du data frame `data_ex` afin de placer la colonne nommée `lanceur` en première position plutôt qu'en dernière, il suffirait d'utiliser l'opérateur `[` et de donner comme valeur au deuxième argument un vecteur contenant les noms des colonnes dans l'ordre désiré, comme dans cet exemple.

```{r}
# Data frame d'origine
data_ex
# Data frame avec colonnes réordonnées
data_ex[, c("lanceur", "de1", "de2")]
```
...
Cette instruction a dit à R de placer la colonne qui est à l'origine en position 3 à la position 1 (car le nombre 3 est en position 1 dans le vecteur fourni au deuxième argument de `[`), la colonne à l'origine en position 1 à la position 2 et finalement celle à l'origine en position 2 à la position 3. Nous pouvons appeler le vecteur `c(3, 1, 2)` de la commande précédente une permutation des données.

Une permutation fournie comme premier argument à `[` (avant la virgule) ordonne les lignes et une permutation fournie comme deuxième argument à `[` (après la virgule, comme dans l'exemple précédent) ordonne les colonnes. Les lignes et les colonnes peuvent être ordonner simultanément. Par exemple, pour déplacer en une commande la dernière observation du data frame `data_ex` en première position et la variable `lanceur` en première position, nous pouvons procéder ainsi.

```{r}
data_ex[c(nrow(data_ex), 1:(nrow(data_ex) - 1)), c("lanceur", "de1", "de2")]
```

Notons que l'opérateur `[` permet aussi de réordonner les éléments d'un vecteur ou d'une liste, ou encore les lignes ou les colonnes d'une matrice. Pour terminer, notons également que les opérateurs d'indiçage `[[` et `$` ne permettent pas de réordonner des éléments, car ils servent à extraire un seul élément.


### Fonction `order`

Pour modifier l'ordre des observations ou des variables dans un jeu de données R, la [fonction `order`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/order.html) est utile. Elle permet d'obtenir un vecteur d'indice des observations dans l'ordre désiré.

Par exemple, pour ordonner les observations du jeu de données `data_ex` en ordre croissant de la valeur de la variable `de1`, 
...
```{r}
permutation <- order(data_ex$de1)
permutation
```

Ce vecteur s'interprète comme suit\ : la plus petite valeur de `data_ex$de1` se trouve en position 4 (valeur 1), les deuxièmes plus petites (valeur 2 qui revient 2 fois) se trouvent dans les positions 1 et 5, ..., la plus grande valeur (6) se trouve en position 8. Ensuite, il suffit de fournir ce vecteur au premier argument de l'opérateur d'indiçage `[` pour ordonner les lignes de `data_ex`. 
...
```{r eval = FALSE}
data_ex[order(data_ex$de1), ]
```

La fonction `order` accepte plus d'un vecteur de valeurs pour déterminer le nouvel ordre. Par exemple, la commande suivante permet d'ordonner les observations de `data_ex` en ordre croissant de valeur de la variable `de1`, en résolvant les égalités en `de1` en considérant l'ordre alphabétique croissant de la variable `lanceur`.

```{r order}
data_ex[order(data_ex$de1, data_ex$lanceur), ]
```

La fonction `order` a un argument `decreasing`, tout comme `sort`, pour demander un ordre décroissant plutôt que croissant. Par exemple, pour modifier l'ordre des colonnes de `data_ex` selon l'ordre alphabétique décroissant des noms de colonnes, nous pourrions procéder comme suit.

```{r}
data_ex[, order(names(data_ex), decreasing = TRUE)]
```

Ainsi, la fonction `order` fournie la permutation selon l'ordre voulue, mais c'est l'opérateur `[` qui effectue la tâche de réordonner des lignes ou des colonnes.
...
Notons que la fonction `arrange` permet seulement d'ordonner les lignes d'un data frame (ou d'un [tibble](https://stt4230.rbind.io/manipulation_donnees/structures_r/#tibbles)), alors que la fonction `order` utilisée avec l'opérateur `[` permet en fait d'ordonner les éléments de n'importe quel type de structure de données et selon n'importe laquelle de ses dimensions.
...
La [fonction `t`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/t.html) sert à transposer (inverser les lignes et les colonnes) une matrice ou un data frame.

```{r t}
data_ex
data_ex_transpo <- t(data_ex)
data_ex_transpo
str(data_ex_transpo)
```
...
Nous pourrions aussi ajuster un modèle linéaire mixte pour réaliser un test équivalent. La [fonction `lme` du package `nlme`](https://stat.ethz.ch/R-manual/R-patched/library/nlme/html/lme.html) ajuste des modèles linéaires mixtes. Elle accepte cependant seulement des jeux de données de mise en forme longue.
...
Revenons maintenant au jeu de données `data_ex`. Celui-ci est sous une mise en forme large, car il contient une seule ligne par individu. Dans ce jeu de données, un individu est un lancer de dés.

```{r}
data_ex
```

Il est possible de faire passer ce jeu de données à une mise en forme longue avec la [fonction `stack`](https://stat.ethz.ch/R-manual/R-patched/library/utils/html/stack.html) comme suit. 

```{r}
data_ex_long <- stack(data_ex)
data_ex_long
```

Cette fonction n'est pas très puissante. Elle n'a pas su traiter la colonne `lanceur` et l'a laissé tomber.

Nous pourrions réaliser la transformation inverse avec la [fonction `unstack`](https://stat.ethz.ch/R-manual/R-patched/library/utils/html/stack.html), pour revenir à la mise en forme originale.

```{r}
data_ex_large <- unstack(data_ex_long)
data_ex_large
```
...
Le package `stats`, chargé en R par défaut, comprend une [fonction nommée `reshape`](https://stat.ethz.ch/R-manual/R-patched/library/stats/html/reshape.html) dont l'utilité est de faire passer un jeu de donnée d'une mise en forme large vers une mise en forme longue, et aussi de réaliser la transformation inverse, d'une mise en forme longue vers une mise en forme large. Cette fonction est puissante, mais un peu difficile à utiliser, il faut l'avouer. Il est facile de se perdre dans ses nombreux arguments.

#### Exemple de transformation vers une mise en forme longue

\mbox{}

Pour faire passer le data frame `data_ex` vers une mise en forme longue avec `reshape`, nous pouvons procéder comme suit.

```{r}
data_ex_long_2 <- reshape(
  data = data_ex, 
  direction = "long", 
  varying = c("de1", "de2"),
  v.names = "resultat"
...
- `idvar`\ : nom(s) de la ou des colonnes pour identifier les individus;
...
Mais supposons que nous n'avons pas créé `data_ex_long_2` avec la fonction `reshape` et que nous voulons transformer ce jeu de données vers une mise en forme large. Afin de réellement simuler une telle situation, nous allons effacer de `data_ex_long_2` son attribut nommé `reshapeLong` comme suit.
...
```{r}
data_ex_large2$IDlancer <- NULL                                 # retirer une colonne
colnames(data_ex_large2)[2:3] <- c("de1", "de2")                # renommer des colonnes
data_ex_large2 <- data_ex_large2[, c("de1", "de2", "lanceur")]  # réordonner les colonnes
rownames(data_ex_large2) <- 1:10                                # renommer les lignes
data_ex_large2
```


#### Récapitulatif concernant les arguments de `reshape`

En résumé, les arguments de `reshape` sont les suivants\ :
...
Voici une commande pour transformer le data frame `data_ex` de sa mise en forme d'origine, large, vers une mise en forme longue avec la fonction `pivot_longer`.

```{r}
data_ex_long_3 <- pivot_longer(
  data = data_ex, 
  cols = c(de1, de2), 
  names_to = "de", 
  values_to = "resultat"
...
Afin d'effectuer une transformation d'une mise en forme longue vers une mise en forme large avec la fonction `pivot_wider` de `tidyr`, le jeu de données fourni en entrée à la fonction doit contenir une colonne pour identifier les individus. Voici donc un exemple d'utilisation de la fonction `pivot_wider`, réalisé en utilisant comme jeu de données d'origine `data_ex_long_2`.

```{r}
data_ex_large_3 <- pivot_wider(
  data = data_ex_long_2,
  names_from = de, 
...
Voici une commande pour transformer le data table `data_ex_dt` de sa mise en forme d'origine, large, vers une mise en forme longue avec la fonction `melt` du package `data.table`.
...
Comme pour la fonction `pivot_wider`, la fonction `dcast` a besoin que le jeu de données à transformer contienne une colonne pour identifier les individus. Voici donc un exemple d'utilisation de la fonction `dcast`, réalisé en utilisant comme jeu de données d'origine le data frame `data_ex_long_2` converti en data table.

```{r}
data_ex_long_2_dt <- as.data.table(data_ex_long_2)
data_ex_large_4 <- dcast(
  data = data_ex_long_2_dt,
...
- Identifier les observations dupliquées\ : `duplicated`,
- Retirer les observations dupliquées\ : `unique`.
- Identifier les observations sans données manquantes\ : `complete.cases`,
- Retirer les observations contenant au moins une donnée manquante\ : `na.omit`.
...
- `[`\ : permet de réordonner les lignes ou les colonnes d'un objet à deux dimensions dans un ordre quelconque;
- `order`\ : retourne la permutation des données requise pour ordonner les données selon l'ordre spécifié $\rightarrow$ pour effectuer l'ordonnancement, il faut fournir le résultat retourné par `order` en argument à l'opérateur `[`
    + argument `decreasing = TRUE` pour ordre décroissant;
...
- `reshape`\ : modifie la mise en forme; les arguments de `reshape` sont les suivants
    ...
    - En lien avec les variables\ :
        ...
        + `times`\ : identifiants des variables.
    - En lien avec les valeurs\ :
        + `v.names`\ : nom(s) colonne(s) contenant les valeurs empilées dans la mise en forme longue ou intermédiaire;
        + `varying`\ : noms colonnes contenant les valeurs dans la mise en forme large ou intermédiaire.
...
- Modification de l'ordre des données\ : `arrange` du package `dplyr`,
    + encadrer les noms de variables (colonnes) d'un appel à la fonction `desc` du package `dplyr` pour obtenir un ordre décroissant.
- Changement de mise en forme de jeux de données\ :
...
- Billet de blogue\ : [A data.table and dplyr tour](https://atrebas.github.io/post/2019-03-03-datatable-dplyr/)
...
- Page web du package sur le CRAN\ : <https://CRAN.R-project.org/package=tidyr>
- Documentation du package\ : <https://tidyr.tidyverse.org/>
...
- Documentation du package\ : <https://rdatatable.gitlab.io/data.table/index.html>
...
- Page web du package sur le CRAN\ : <https://CRAN.R-project.org/package=stringr>
- Documentation du package\ : <https://stringr.tidyverse.org/>
...
- Documentation R\ : [`help(regexp)`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/regex.html)
- Page Wikipédia\ : [Expression régulière](https://fr.wikipedia.org/wiki/Expression_r%C3%A9guli%C3%A8re)
- Goulet, V. (2020). *Programmer avec R*. Université Laval, École d'actuariat. Section 11.3. URL <https://vigou3.gitlab.io/programmer-avec-r/>
...
- Billet de blogue\ : [How to work with strings in base R - An overview of 20+ methods for daily use](https://jozefhajnala.gitlab.io/r/r007-string-manipulation/)


#### Pour aller plus loin en manipulation de dates\ :  {-}

- Documentation R à propos de tous les format possibles\ : [`help(strptime)`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/strptime.html),
- Billet de blogue\ : [Using Dates and Times in R](http://www.noamross.net/blog/2014/2/10/using-times-and-dates-in-r---presentation-code.html)  

Package `lubridate`\ :

- Page web du package sur le CRAN\ : <https://CRAN.R-project.org/package=lubridate>
- Documentation du package\ : <https://lubridate.tidyverse.org/>
- Vignette\ : [Do more with dates and times in R](https://cran.r-project.org/web/packages/lubridate/vignettes/lubridate.html)
