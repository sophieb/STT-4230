...
Pour concaténer les jeux de données `data_ex` et `supp` en un seul jeu de données, nous pourrions utiliser la [fonction `rbind`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/cbind.html) comme suit\ :
...
Pour concaténer en colonnes les jeux de données `data_ex` et `data_ex_lanceurs`, nous pourrions utiliser la [fonction `cbind`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/cbind.html) ou `data.frame` comme suit.
...
Les opérateurs de comparaison vus dans les [notes sur les calculs mathématiques en R](https://stt4230.rbind.io/calculs/calculs_math_r/) permettent de comparer les éléments d'objets R. Mais comment comparer des objets entiers? Cela dépend de ce qui doit être comparé.
