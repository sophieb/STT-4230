...
*Note préliminaire : Lors de leur dernière mise à jour, ces notes ont été révisées en utilisant R 4.0.3, le package `tibble` version 3.0.5 et le package `data.table` version 1.13.6. Pour d'autres versions, les informations peuvent différer.*
...
Il existe une fonctions raccourcie pour l'utilisation de la fonction `paste` avec `sep = ""` (aucun caractère pour séparer les chaînes de caractères)&nbsp;: la fonction `paste0`. 
...
Rappel des références pour en apprendre davantagee à propos des data tables&nbsp;:
