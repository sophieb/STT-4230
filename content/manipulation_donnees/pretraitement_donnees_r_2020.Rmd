...
Cependant, il est plus simple d'utiliser encore une fois la fonction `na.omit`. Pour retirer la varaible `lanceur`, l'argument `select` de la fonction `subset` peut être spécifié comme dans la commande suivante, qui accompli la tâche souhaitée.
...
Tout comme le fait la fonction `subset`, les lignes pour lesquelles la condition vaut `NA` n'ont pas été conservées. Pour retirer les observations contenant des données manquantes, nous pouvons utiliser `na.omit`, ou encore [`drop_na`](https://tidyr.tidyverse.org/reference/drop_na.html), une fonction de similaire contenue dans le package `tidyr`.
...
Pour effectuer de la sélection de sous-ensembles de données avec la package `data.table`, il faut exploiter son [opérateur `[` et ses arguments `i` et `j`](https://rdatatable.gitlab.io/data.table/reference/data.table.html). Après avoir transformé le data frame `dataEx` en data table, 
...
Pour effectuer le ordonnancement des éléments d'un vecteur (ou un facteur) selon les valeurs des données en éléments, de façon à replacer les éléments en ordre croissant ou décroissant de valeur, il faut plutôt utiliser la [fonction `sort`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/sort.html). Par exemple, ordonnons des valeurs numériques en ordre décroissant.
...
Fonctions pour la transformation de mise en forme :
...
- Sélection de sous-ensembles de données : `filter` et `select` du package `dplyr`.
- Combinaison de données :
...
- Modification de l’ordre des données : `arrange` du package `dplyr`,
    + encadrer les noms de variable d'un appel à la fonction `desc` du package `tidyr` pour obtenir un ordre décroissant.
- Changement de mise en forme de jeux de données :
...
- Sélection de sous-ensembles de données : opérateur `[`.
- Combinaison de données :
