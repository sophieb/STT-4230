...
Ainsi, dans une commande R, le chemin d'accès `C:\coursR\data_ex.txt` devrait être écrit ainsi : `"C:/coursR/data_ex.txt"`. Ce chemin pourrait aussi être produit avec la [fonction `file.path`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/file.path.html) comme suit.
...
Les fonctions les plus simplistes en R pour lire dans des fichiers texte externes sont [`readLines`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/readLines.html) et [`scan`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/scan.html). Cependant, ces fonctions ne sont pas les meilleurs outils pour la lecture de tableaux de données.
...
Pour l'importation d'un tableau de données provenant d'un fichier texte, la meilleure fonction offerte dans le R de base est plutôt [`read.table`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/read.table.html) du package `utils` (chargé par défaut à l'ouverture d'une session R). Par exemple, lisons les données contenues dans le fichier `data_ex.txt` avec `read.table`.
...
Nous avons réglé le problème de lecture de la deuxième colonne. Attardons-nous maintenant à la dernière colonne. Elle est enregistrée dans un vecteur de chaînes de caractères. Si nous avions préféré que `read.table` transforme les colonnes contenant des valeurs caractères en facteur, il aurait fallu assigner la valeur `TRUE` à l'argument `stringsAsFactors`. Avant sa version 4.0.0, R avait ce comportement par défaut. Le nouveau comportement par défaut correspond à ce que nous désirons ici.

Nous avons illustré deux arguments de la fonction `read.table`. Elle en possède plusieurs autres, à adapter selon le fichier à lire. 
...
-	Est-ce qu’il y a des caractères spéciaux, par exemple des accents ? Si oui, quel encodage est utilisé ?
    + Un des encodages les plus courants est `UTF-8`. En cas de problèmes avec des accents mal lus, essayez l'argument `encoding = "UTF-8"` ou `fileEncoding = "UTF-8"`. Ça pourrait régler le problème.
...
-	Est-ce que les colonnes contenant des chaînes de caractères doivent être converties en facteurs ?
    ...
    + Par défaut `stringsAsFactors = FALSE` (depuis la version 4.0.0 de R, pour des versions antérieures de R la valeur par défaut est `TRUE`).
...
Le format de fichier le plus universel pour exporter des données est le format texte. Un grand nombre de logiciels peuvent lire ce format. Les fonctions de base en R pour écrire dans un fichier texte sont [`writeLines`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/writeLines.html), [`write`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/write.html) et [`write.table`](https://stat.ethz.ch/R-manual/R-patched/library/utils/html/write.table.html). Les fonctions `writeLines` et `write` sont l'équivalent de `readLines` et `scan`, respectivement, pour l'écriture dans des fichiers au lieu de la lecture. Pour réaliser une tâche d'exportation de jeux de données, la fonction `write.table` est la mieux adaptée des trois. Par exemple, exportons le contenu du data frame `data_ex` dans un fichier texte nommé `data_ex_export.txt` et positionné dans le répertoire de travail.

```{r exportTexte, eval = FALSE}
write.table(x = data_ex, file = "data_ex_export.txt", fileEncoding = "UTF-8")
```

Il faut spécifier à la fonction l'objet, typiquement une matrice ou un data frame, dans lequel se trouvent les données à exporter (argument `x`), ainsi que le fichier dans lequel écrire les données (argument `file`). Le fichier peut être spécifié par un chemin d'accès complet ou relatif, comme pour les fonctions de lecture de fichier.

Tout comme `read.table`, la fonction `write.table` possède plusieurs arguments permettant de contrôler le formatage des données. Un de ces arguments, nommé `fileEncoding`, est utilisé dans l'exemple précédent. En fournissant la valeur `"UTF-8"` à l'argument `fileEncoding`, on crée un fichier d'encodage `UTF-8`, dont l'usage est très répandu. Sans cet argument, `write.table` pourrait par défaut créer un fichier dont l'encodage est spécifique au système d'exploitation utilisé et qui pourrait causer problèmes sous un autre système d'exploitation. Par exemple, si vous travaillez sous Windows, le fichier créé pourrait contenir des lettres accentuées non reconnues sous macOS en raison d'un encodage mal compris. Spécifier l'argument `fileEncoding = "UTF-8"` permet d'éviter ce genre de souci.

La fonction `write.table` possède d'autres arguments, tel que notamment `dec` pour spécifier le symbole de décimale, `na` pour spécifier le symbole représentant les valeurs manquantes, `sep` pour spécifier le séparateur entre les champs, `row.names` et `col.names` pour indiquer si les noms des lignes et des colonnes, respectivement, doivent être inclus dans le fichier créé. Tous les informations concernant ces arguments, ainsi que tous les autres arguments non mentionnés ici, se trouvent dans la [fiche d'aide de la fonction `write.table`](https://stat.ethz.ch/R-manual/R-patched/library/utils/html/write.table.html).

Il existe aussi deux fonctions enveloppe à la fonction `write.table` pour les formats CSV, nommées [`write.csv` et `write.csv2`](https://stat.ethz.ch/R-manual/R-patched/library/utils/html/write.table.html).


## Fichier JSON

Pour exporter les données dans un data frame en format JSON avec le [package `jsonlite`](https://CRAN.R-project.org/package=jsonlite), il faut d'abord transformer les données au format JSON avec la fonction `toJSON`. Ensuite, l'objet obtenu doit être écrit dans un fichier externe avec la fonction `write` du R de base.
...
Le [package `openxlsx`](https://ycphs.github.io/openxlsx/index.html) offre des fonctions pour exporter des données stockées dans un objet R dans un fichier EXCEL. Par exemple, le contenu du data frame `data_ex` peut être écrit dans un fichier EXCEL avec la [fonction `write.xlsx`](https://ycphs.github.io/openxlsx/reference/write.xlsx.html) comme suit.
