...
La première dimension d'un data frame est la longueur commune des objets qu'il contient (dans l'exemple 4) et sa deuxième dimension est le nombre d'objets qu'il contient (dans l'exemple, le data frame contient 3 vecteurs).
...
*__-__ positions des éléments à éliminer*;
...
Un de ces cas est l'extraction d'une colonne, complète ou partielle, d'un data frame. Le résultat de l'extraction est alors un vecteur ou un facteur, et nom un autre data frame, comme dans cet exemple.
...
nrow(mat)
```
La taille de sa première dimension est vue comme un nombre de lignes. Il en a `r nrow(mat)`.
```{r}
ncol(mat)
```
La taille de sa deuxième dimension est vue comme un nombre de colonnes. Il en a `r ncol(mat)`.
```{r}
length(mat)
```
