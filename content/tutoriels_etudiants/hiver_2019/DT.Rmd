...
```{r 1er tableau, eval = FALSE}
datatable(data = populationVillesCanada)
```
```{r 1er tableau widgetframe, echo = FALSE, eval = TRUE}
t1 <- datatable(data = populationVillesCanada)
widgetframe::frameWidget(t1)
```
...
```{r no rownames, eval = FALSE}
datatable(populationVillesCanada, rownames = FALSE,
          options = list(pageLength = 5))
          # L'option ajoutée sert à restreindre le nombre de lignes affichées.
```
```{r no rownames widgetframe, echo = FALSE, eval = TRUE}
t_lignes <- datatable(populationVillesCanada, rownames = FALSE,
          options = list(pageLength = 5))
widgetframe::frameWidget(t_lignes, height = 320)
```

### Colonne par substitution {-}
Le nom de colonne `VariationPopulation` est recherché et remplacé par `NouveauNom`:
```{r colnames par substitution, eval = FALSE}
datatable(populationVillesCanada, colnames = c('NouveauNom' = 'VariationPopulation'),
          options = list(pageLength = 5))
```
```{r colnames par substitution widgetframe, echo = FALSE, eval = TRUE}
t_substitut <- datatable(populationVillesCanada, colnames = c('NouveauNom' = 'VariationPopulation'),
          options = list(pageLength = 5))
widgetframe::frameWidget(t_substitut, height = 320)
```

### Colonnes par vecteur de caractères {-}
Les noms des colonnes sont modifiés par un vecteur de chaînes de caractères.
```{r colnames par vecteur de caractères, eval = FALSE}
datatable(populationVillesCanada, colnames = c('Col1', 'Col2', 'Col3', 'Col4'),
          options = list(pageLength = 5))
```
```{r colnames par vecteur de caractères widgetframe, echo = FALSE, eval = TRUE}
t_col <- datatable(populationVillesCanada, colnames = c('Col1', 'Col2', 'Col3', 'Col4'),
          options = list(pageLength = 5))
widgetframe::frameWidget(t_col, height = 320)
```
...
datatable(data = populationVillesCanada, rownames = FALSE, class = "hover",
          options = list(pageLength = 5))
```
```{r hover widgetframe, echo = FALSE,,  eval = FALSE}
t_hover <- datatable(data = populationVillesCanada, rownames = FALSE, class = "hover",
          options = list(pageLength = 5))
widgetframe::frameWidget(t_hover, height = 320)
```

### `cell-border` {-}
Les cellules sont entourées de lignes. 
```{r cell-border, eval = FALSE}
datatable(data = populationVillesCanada, 
          rownames = FALSE, 
          class = "hover cell-border", # La modification est combinée à la précédente.
          options = list(pageLength = 5))
```
```{r cell-border widgetframe, echo = FALSE, eval = TRUE}
t_border <- datatable(data = populationVillesCanada, 
          rownames = FALSE, 
          class = "hover cell-border", # La modification est combinée à la précédente.
          options = list(pageLength = 5))
widgetframe::frameWidget(t_border, height = 320)
```

### `compact` {-}
Le vide dans les cellules est réduit afin d'optimiser l'espace.
```{r compact, eval = FALSE}
datatable(data = populationVillesCanada, 
          rownames = FALSE, 
          class = "hover cell-border compact",
          options = list(pageLength = 5))
```
```{r compact widgetframe, echo = FALSE, eval = TRUE}
t_compact <- datatable(data = populationVillesCanada, 
          rownames = FALSE, 
          class = "hover cell-border compact",
          options = list(pageLength = 5))
widgetframe::frameWidget(t_compact, height = 320)
```
...
datatable(populationVillesCanada, 
          ...
          options = list(pageLength = 5))
```
```{r caption widgetframe, echo = FALSE, eval = TRUE}
t_titre <- datatable(populationVillesCanada, 
          rownames = FALSE, 
          class = "hover cell-border compact", 
          caption = "Tableau 1: Estimations de la population, selon la région métropolitaine de recensement.",
          options = list(pageLength = 5))
widgetframe::frameWidget(t_titre, height = 320)
```
...
```{r caption et filter, eval = FALSE}
datatable(populationVillesCanada, 
          rownames = FALSE, 
          class = "hover cell-border compact", 
          caption = "Tableau 1: Estimations de la population, selon la région métropolitaine de recensement.",
          ...
          options = list(pageLength = 5))
```
```{r caption et filter widgetframe, echo = FALSE, eval = TRUE}
t_filtre <- datatable(populationVillesCanada, 
          rownames = FALSE, 
          class = "hover cell-border compact", 
          caption = "Tableau 1: Estimations de la population, selon la région métropolitaine de recensement.",
          filter = 'top', # Les filtres sont positionnés au-dessus des colonnes.
          options = list(pageLength = 5))
widgetframe::frameWidget(t_filtre, height = 320)
```

### Tableau modifiable {-}
Il suffit de double-cliquer dans une cellule pour modifier la valeur.
```{r caption, filter et editable, eval = FALSE}
datatable(populationVillesCanada, 
          rownames = FALSE,
          class = "hover cell-border compact", 
          caption = "Tableau 1: Estimations de la population, selon la région métropolitaine de recensement.",
          ...
          options = list(pageLength = 5))
```
```{r caption, filter et editable widgetframe, echo = FALSE, eval = TRUE}
t_modif <- datatable(populationVillesCanada, 
          rownames = FALSE,
          class = "hover cell-border compact", 
          caption = "Tableau 1: Estimations de la population, selon la région métropolitaine de recensement.",
          filter = 'top', 
          editable = TRUE,
          options = list(pageLength = 5))
widgetframe::frameWidget(t_modif, height = 320)
```
...
```
```{r language widgetframe, echo = FALSE, eval = TRUE}
t_langue <- datatable(populationVillesCanada, 
          options = list(
            language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/French.json'),
            pageLength = 5
            ))
widgetframe::frameWidget(t_langue, height = 320)
```
...
```
```{r order widgetframe, echo = FALSE, eval = TRUE}
t_order <- datatable(populationVillesCanada, 
          options = list(
            language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/French.json'),
            order = list(list(4, 'desc'), # Tri décroissant sur la colonne 4 (VariationPopulation).
                         list(1, 'asc')) # Tri secondaire croissant sur la colonne 1.
            ))
widgetframe::frameWidget(t_order)
```
...
```
```{r formatRound widgetframe, echo = FALSE, eval = TRUE}
t_format <- datatable(populationVillesCanada)  %>% 
  formatRound('VariationPopulation', 0) # Les valeurs de cette colonne seront arrondies à l'entier.
widgetframe::frameWidget(t_format)
```
...
```
```{r formatStyle widgetframe, echo = FALSE, eval = TRUE}
t_style <- datatable(populationVillesCanada)  %>% 
  formatStyle(               # Fonction pour changer le style d'une colonne
    'Ville',                 # Nom de la colonne à modifier
    color = 'white',         # Couleur du texte dans la colonne
    backgroundColor = 'red', # Couleur des cases de la colonne
...
widgetframe::frameWidget(t_style)
```
