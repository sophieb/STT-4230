...
|:---------|:-----------------|:-------------------------------|:----------------------|
| [`transition_*`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/transition_states) | states / time / reveal / events / filter / layers / components / manual / null | Contrôle les données à animer | Oui |
| [`view_*`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/view_follow)  | follow / step / step_manual / zoom / zoom_manual / static | Contrôle le mouvement du plan de vue | Oui |
| [`shadow_*`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/shadow_mark) | wake / trail / mark / null |Contrôle la persistance des données | Oui |
| [`enter/exit_*`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/enter_exit) | manual / appear / disappear / fade / grow / shrink / recolour / recolor / fly / drift / reset | Contrôlent la façon dont les données apparaissent et disparaissent | Oui |
| [`animate`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/animate) | - | Contrôle la visualisation des tableaux (*frames*) de l'animation et leur nombre | Non |
| [`anim_save`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/anim_save) | - | Permet la sauvegarde de l'objet animé dans un fichier | Non |
...
**NOTE 2:** La description exhaustive de chaque fonction du package `gganimate` est disponible sur le [site web de gganimate](https://gganimate.com/reference/index.html?fbclid=IwAR1KV7Ka1bD5ul1udL3tcAqLjccmx2T8FrgW_RYkxIh7gyzLWsYWb3F30xc).
...
- Avec la fonction [`shadow_mark`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/shadow_mark), nous avons empêché que les points d'une seule région apparaissent et disparaissent en mettant les deux arguments `past` et `future` à `FALSE`. Finalement, avec la fonction `animate`, nous avons retiré la pause (argument `end_pause`) à la fin de l'animation pour que le tout soit fluide.  

### Pour s'amuser encore un peu avec `quakes`

Une autre fonction intéressante du package `gganimate` est [`transition_layers`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/transition_layers). Cette dernière permet d'ajouter graduellement chaque couche du graphique. Par exemple, il est possible de faire apparaître des diagrammes en boîte, puis d'ajouter les points qui les constituent par la suite.
...
- [`transition_layers`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/transition_layers) permet de faire apparaître les diagrammes en boîte, puis les points qui les constituent. Dans cet exemple, `geom_boxplot` est une couche et `geom_jitter` est une autre couche. L'argument `layer_length` permet de déterminer le temps de pause avant l'ajout de la prochaine couche et l'argument `transition_length` permet de contrôler la durée de l'entrée d'une nouvelle couche.

- [`enter_grow`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/enter_exit) et [`enter_fade`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/enter_exit) permettent de contrôler l'apparition des différentes couches. Comme dans cet exemple, il est possible de combiner plusieurs fonctions de type [`enter_*`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/enter_exit) afin d'obtenir l'animation souhaitée.
...
- [`transition_reveal`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/transition_reveal) calcule des états intermédiaires entre les années (`Year`) pour tracer une ligne en continu.
- [`view_follow`](https://www.rdocumentation.org/packages/gganimate/versions/1.0.5/topics/view_follow) initie un mouvement du plan de vue selon la progression des axes. Ici, nous ne voulions pas que le plan de vue suive l'axe des abscisses. Nous avons donc donné la valeur `FALSE` à l'argument `fixed_x`.
