...
L'installation du package `plotly` dans votre librarie R s'effectue avec la commande suivante :
...
Afin de montrer l'utilisation du package, nous utiliserons le dataset `quakes` et lui ajouter 2 facteurs.
```{r}
head(quakes, n = 5)
...
`plotly` a été concu afin de construire les figures graphiques de façon séquentielle. Pour ce faire, le résultat d'une opération est passé à la suivante. Il est possible de créer des variables temporaires et de les passer aux fonctions suivantes, mais nous recommandons l'utilisation de l'opérateur `%>%`.
...
Pour créer un histrogamme, l'argument 'type' doit recevoir la valeur `histogram`.
```{r}
...
`ggplotly` est une fonction du package `plotly` qui permet de tranformer un graphique réalisé avec `ggplot2` en un graphique de type `plotly` afin de le rendre interactif. Voyons un exemple sur les données `quakes` : nous créons, dans un premier temps, le graphique avec `ggplot2`.
...
Il est aussi possible d'utilise `subplot` pour des graphiques à trois dimensions tel que montré sur cette page: <https://plot.ly/r/3d-subplots/>.
...
Ici, nous avons utilisé l'attribut `region` afin de colorier les points sans produire deux séries de données différentes. De plus, l'argument `type` serait optionel, car `plotly` produirait par défaut un graphique de dispersion lorsque `x`, `y` et `z`sont des vecteurs de valeurs.
...
params <- list(
  ...
  projection = list(type = 'equirectangular', 
                    rotation = list(lon = 120)),  # Rotation de la carte pour voir 
...
Les pages suivantes contiennent plus d'information et d'exemples quant à la production de carte géographiques:
...
Un exemple montrant l'incroyable gain de performance obtenu grace à l'utilisation de `scattergl` est disponible [ici](https://plot.ly/r/compare-webgl-svg/).
